...
1. By adapting where the file is read from you could choose a new csv file to pull information from. We would replace the election_results.csv to another file name. We would also adjust the pathing to either place this new csv file into the resources folder or change the "Resources" section to reflect the new folder path. `file_to_load = os.path.join("Resources", "election_results.csv")` At the same time, we would adjust the output as well. `file_to_save = os.path.join("analysis", "election_analysis.txt")` In the same manner we would make a new txt file and adjust its pathing accordingly
